delta_theta <- function(xt,mu,V,prop,l,param){
  ...
  if (l != p){
    delta_pi[l] = (dmvnorm(xt,mean=mu[,l],sigma=V[,,l]) * (sum(param$gt) - param$gt[l] + prop[l]*dmvnorm(xt,mean=mu[,p],sigma=V[,,p])))/(sum(param$gt))^2
    for (a in setdiff(c(1:(p-1)),l)){
      delta_pi[a] = param$gt[l]*(dmvnorm(xt,mean=mu[,p],sigma=V[,,p]) - dmvnorm(xt,mean=mu[,a],sigma=V[,,a]))/(sum(param$gt))^2
    }
  }else if(l == p){
    for (a in setdiff(c(1:(p-1)),l)){
      delta_pi[a] = (-dmvnorm(xt,mean=mu[,p],sigma=V[,,p])*(sum(param$gt) - param$gt[l]) - param$gt[l]*dmvnorm(xt,mean=mu[,a],sigma=V[,,a]))/(sum(param$gt))^2
    }
  }
